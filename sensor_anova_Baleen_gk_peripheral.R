sensor_anova_Baleen_gk_peripheral <-function(filePrefix,t1,t2){
...
outfile <-paste(filePath,filePrefix,".Baleen_All.",t1,"-",t2,"_anova_gk_peripheral.txt",sep="")
sink(outfile);
...
#compute the ANOVA
eztest <- ezANOVA(data=erpData.quad.frontal,dv = .(amp),wid=.(subj),within=.(prime,prop,hemCode),type=3,detailed=TRUE)
...
#compute the ANOVA
eztest <- ezANOVA(data=erpData.quad.parietal,dv = .(amp),wid=.(subj),within=.(prime,prop,hemCode),type=3,detailed=TRUE)
...
#compute the ANOVA
eztest <- ezANOVA(data=erpData.quad.left.frontal,dv = .(amp),wid=.(subj),within=.(prime,prop),type=3,detailed=TRUE)
...
#compute the ANOVA
eztest <- ezANOVA(data=erpData.quad.left.parietal,dv = .(amp),wid=.(subj),within=.(prime,prop),type=3,detailed=TRUE)
...
#compute the ANOVA
eztest <- ezANOVA(data=erpData.quad.right.frontal,dv = .(amp),wid=.(subj),within=.(prime,prop),type=3,detailed=TRUE)
...
#compute the ANOVA
eztest <- ezANOVA(data=erpData.quad.right.parietal,dv = .(amp),wid=.(subj),within=.(prime,prop),type=3,detailed=TRUE)
