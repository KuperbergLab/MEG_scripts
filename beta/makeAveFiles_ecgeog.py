...
##	expList = ['Blink', 'ATLLoc','MaskedMM','BaleenLP','BaleenHP','AXCPT']
        expList = ['BaleenHP' 'BaleenLP'] ## changed on 9/7 for testing number of trials in each case(rej, PCA+rej, PCA+norej -CU)
	...
	for exp in expList:
                 if exp == 'ATLLoc'or exp =='MaskedMM'or exp=='BaleenLP'or exp=='BaleenHP'or exp=='AXCPT':
                        for run in runDict[exp]:
                                ...
                                myFile.write('\tlogfile\t\t./logs/ssp_avelogs/'+subjID + '_' + exp + run + '_ecgeog1-ave.log\n')
                                if projType == 'projoff':
