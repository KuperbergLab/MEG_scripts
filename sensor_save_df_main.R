...
erpData.all<-data.frame(subj=factor(buffer$V5),subjGroup=factor(buffer$V4),exp=factor(buffer$V6),cond=factor(buffer$V7),elec=factor(buffer$V3),apCode=factor(buffer$V1),chanLabel=factor(buffer$V2),amp=buffer$V8)
}
