...
foreach proj ( 'projon' 'projoff') ## Do not change the order. 
	...
	python /cluster/kuperberg/SemPrMM/MEG/scripts/makeAveFiles_ecgeog.py $1 $proj >>& $log
...
#  
 	...
 	--gave $1_BaleenLP_All_ecg-ave.fif \
 	--$proj --lowpass 20 >>& $log
#
#
	...
	--gave $1_BaleenHP_All_ecg-ave.fif \
	--$proj --lowpass 20 >>& $log
...
# 	endif
    ...
    if ( $proj == 'projon' ) then
	        ...
	        python /cluster/kuperberg/SemPrMM/MEG/scripts/rej_rej2eve_projon_ecgeog.py $1 >>& $log
	        echo "done" >>& $log
    endif  
