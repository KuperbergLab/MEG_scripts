...
def makeAveFiles_ecgeog_norej(subjID,projType):
	...
	for exp in expList:
                 if exp == 'ATLLoc'or exp =='MaskedMM'or exp=='BaleenLP'or exp=='BaleenHP'or exp=='AXCPT':
                        for run in runDict[exp]:
                                ...
                                myFile.write('\tlogfile\t\t./logs/ssp_avelogs/'+subjID + '_' + exp + run + '_ecgeog1_norej-ave.log\n')
                                if projType == 'projoff':
...
if __name__ == "__main__":
	makeAveFiles_ecgeog_norej(sys.argv[1],sys.argv[2])
