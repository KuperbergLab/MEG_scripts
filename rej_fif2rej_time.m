...
for chan = chan_c
    ...
    %first_time = (ds.first_samp/ds.info.sfreq);
    %last_time = (ds.last_samp/ds.info.sfreq);
    
    X = fiff_read_raw_segment(ds,ds.first_samp,ds.last_samp,chans);
   % X = fiff_read_raw_segment_times(ds,first_time,last_time,chans); 
    ...
    dlmwrite(new_path, bad_mat, 'delimiter', '\t', 'precision', '%3.3f');
...
N = length(X);
w = 60; %each window is 60 samples long - default 60
mat_ind = 1;
bad_mat = zeros(N,2);
...
for ii = n
    ...
    if d > thr || d < -thr
        tp = first + ii;
        time = double(tp/ds.info.sfreq);
        bad_mat(mat_ind,:) = [time chans];
        mat_ind = mat_ind + 1;
    end
end
...
function bad_mat = flat(X, thr, ds, chans, p, first_time)
...
function bad_mat = peak2peak(X, thr, ds, chans, p, first_time)
