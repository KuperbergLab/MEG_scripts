...
#usage: subjType: sc ac or ya (in Lower case) 'ATLLoc' 'BaleenHP' 'BaleenLP' 'MaskedMM' 'AXCPT'- (only in the case of ya)
...
cd /autofs/cluster/kuperberg/SemPrMM/MEG/data/
   foreach i ( $1* )
     ...
     rm {$i}_MEGArtReject-BadChanAXCPT
   end 
   cd ../../..
    ...
    foreach i ($1*)
      ...
      if (-e /autofs/cluster/kuperberg/SemPrMM/MEG/data/$i/ave_projon/logs/{$i}_AXCPTRun1-ave.log) then 
          foreach t ( {$i}_AXCPTRun?-ave.log ) 
                  rm {$i}_MEGArtReject{$t}
                  rm {$i}_MEGArtReject-BadChan{$t}
...
cd /autofs/cluster/kuperberg/SemPrMM/MEG/results/MEGArtRej
   ...
   rm $1_MEGArtRejSummaryAXCPT

echo 'ComputeEvents'
cd /autofs/cluster/kuperberg/SemPrMM/MEG/data/
foreach par ( 'AXCPT' 'ATLLoc' 'BaleenHP' 'BaleenLP' 'MaskedMM' )
   echo $par
   python /cluster/kuperberg/SemPrMM/MEG/scripts/computeEvents.py $1 $par /cluster/kuperberg/SemPrMM/MEG/results/MEGArtRej/$1_MEGArtRejSummary-$par
end
